...
We prove the following lemma in order to show that the output of $C$ has sufficient min-entropy to serve as the distribution $\mathcal{D}$ in the \textsf{DLWE}($\mathcal{D}$) problem.

\begin{lemma}
Let $S_1$ be $(n+k - \ell)$ dimensional, where $0 < \ell < k$. Under certain conditions on $n$, $k$ and $\ell$, there are at least exponentially many matrices $M$ which the first step of $C$ could sample.
\end{lemma}
